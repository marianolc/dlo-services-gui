export const CHANGE_LANGUAGE = "CHANGE_LANGUAGE";

export const AUTH_SIGN_OUT = "sign_out";

export const AUTH_REQUEST = "AUTH_REQUEST";
export const AUTH_SUCCESS = "AUTH_SUCCESS";
export const AUTH_FAILURE = "AUTH_FAILURE";

export const LIST_REQUEST = "LIST_REQUEST";
export const LIST_FAILURE = "LIST_FAILURE";

export const VIEW_REQUEST = "VIEW_REQUEST";
export const VIEW_SUCCESS = "VIEW_SUCCESS";

export const LIST_SUCCESS = "LIST_SUCCESS";
export const VIEW_FAILURE = "VIEW_FAILURE";

export const VIEW_UPDATE = "VIEW_UPDATE";

export const INVENTORY_MODEL_SUCCESS = "INVENTORY_MODEL_SUCCESS";
export const INVENTORY_MODELS_SUCCESS = "INVENTORY_MODELS_SUCCESS";

export const INVENTORY_FAMILIES_SUCCESS = "INVENTORY_FAMILIES_SUCCESS";
